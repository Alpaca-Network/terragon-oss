...
- **Claude Opus 4.6**: Anthropic's newest flagship model is now available. Opus 4.6 brings significant improvements in coding, agentic tasks, and reasoning, including a 68.8% score on ARC AGI 2 (up from 37.6% for Opus 4.5).
- **Agent Teams (Preview)**: Enable experimental Agent Teams support for Claude Code. When enabled, the agent can coordinate multiple Claude Code instances working together as a team with shared tasks and inter-agent messaging. Enable this feature in [Settings](https://www.terragonlabs.com/settings/preferences) under Preview Features.
- **1M Context Window**: Opus 4.6 is the first Opus model to support a 1 million-token context window.
